...
def _on_startup():
    global _scheduler
    try:
        from readreceipt.cli import _do_purge_live_blogs
        log.info("running one-time live-blog purge at startup")
        _do_purge_live_blogs(engine, dry_run=False)
    except Exception:
        log.exception("startup live-blog purge failed (continuing)")
