# ReadReceipt v1 Implementation Plan
...
**Backend (Python package `readreceipt/`):**
- `readreceipt/__init__.py` — package marker
- `readreceipt/config.py` — env var loading, single source for `DATABASE_URL`, `ANTHROPIC_API_KEY`, `ENVIRONMENT`
- `readreceipt/url_utils.py` — URL canonicalization (`canonicalize_url`)
- `readreceipt/storage.py` — SQLAlchemy `Base`, models (`Article`, `Version`, `Change`), session factory, helper queries
- `readreceipt/scraper.py` — fetch URL with httpx, extract body via trafilatura, extract headline from `og:title`
- `readreceipt/differ.py` — `compute_diff`, `should_skip_llm` pre-filter
- `readreceipt/classifier.py` — Anthropic SDK call to Haiku, structured tool-use output, validation
- `readreceipt/pipeline.py` — orchestrates per-article scrape flow (the diagram in spec §Scraping Pipeline)
- `readreceipt/discovery.py` — RSS feed parsing and new-URL detection
- `readreceipt/scheduler.py` — APScheduler tick that calls discovery + re-scrape
- `readreceipt/api.py` — FastAPI routes
- `readreceipt/main.py` — entry point, mounts static frontend, starts scheduler
- `readreceipt/cli.py` — CLI with `--dry-run`
...
- Create: `.env.example`
- Create: `pytest.ini`
- Create: `readreceipt/__init__.py`
- Create: `readreceipt/config.py`
...
DATABASE_URL=sqlite:///./readreceipt.db
ENVIRONMENT=dev
```
...
- [ ] **Step 5: Create `readreceipt/__init__.py` (empty)**
...
Expected: FAIL — `module 'readreceipt' has no attribute 'config'` or `ImportError`.

- [ ] **Step 10: Implement `readreceipt/config.py`**
...
```bash
git add .gitignore requirements.txt .env.example pytest.ini readreceipt/ tests/ feeds.yaml
...
**Files:**
- Create: `readreceipt/url_utils.py`
...
- [ ] **Step 3: Implement `readreceipt/url_utils.py`**
...
```bash
git add readreceipt/url_utils.py tests/test_url_utils.py
...
- [ ] **Step 3: Implement `readreceipt/storage.py`**
...
```bash
git add readreceipt/storage.py tests/test_storage.py
...
**Files:**
- Modify: `readreceipt/storage.py` (add helper functions)
...
- [ ] **Step 3: Append helpers to `readreceipt/storage.py`**
...
- [ ] **Step 3: Implement `readreceipt/differ.py`**
...
```bash
git add readreceipt/differ.py tests/test_differ.py
...
- [ ] **Step 3: Implement `readreceipt/scraper.py`**
...
def fetch_url(url: str, *, timeout: float = 15.0) -> Optional[str]:
    headers = {
        "User-Agent": (
            "ReadReceiptBot/1.0 "
            "(+https://github.com/yourname/readreceipt; contact: you@example.com)"
        )
    }
    try:
...
```bash
git add readreceipt/scraper.py tests/test_scraper.py
...
**Files:**
- Create: `readreceipt/classifier.py`
...
- [ ] **Step 3: Implement `readreceipt/classifier.py`**
...
```bash
git add readreceipt/classifier.py tests/test_classifier.py
...
**Files:**
- Create: `readreceipt/pipeline.py`
...
- [ ] **Step 3: Implement `readreceipt/pipeline.py`**
...
```bash
git add readreceipt/pipeline.py tests/test_pipeline.py
...
**Files:**
- Create: `readreceipt/discovery.py`
...
- [ ] **Step 3: Implement `readreceipt/discovery.py`**
...
```bash
git add readreceipt/discovery.py tests/test_discovery.py
...
**Files:**
- Modify: `readreceipt/storage.py` (add `articles_due_for_rescrape`)
- Modify: `tests/test_storage.py` (add tests)
- Create: `readreceipt/scheduler.py`
...
- [ ] **Step 3: Append helper to `readreceipt/storage.py`**
...
- [ ] **Step 7: Implement `readreceipt/scheduler.py`**
...
```bash
git add readreceipt/storage.py readreceipt/scheduler.py tests/test_storage.py tests/test_scheduler.py
...
- [ ] **Step 3: Implement `readreceipt/cli.py`**
...
```bash
git add readreceipt/cli.py tests/test_cli.py
...
Find a current Guardian, BBC, and NPR article URL by hand (or by running `python -m readreceipt.cli --dry-run` and copying any URL). Then in a Python REPL:
...
from bs4 import BeautifulSoup
from readreceipt.scraper import extract_headline
...
for outlet, url in urls.items():
    print(f"=== {outlet} ===")
    html = httpx.get(url, headers={"User-Agent": "ReadReceiptBot/1.0"}, timeout=15.0, follow_redirects=True).text
    headline = extract_headline(html)
    body = trafilatura.extract(html, include_comments=False, include_tables=False)
...
**Files:**
- Create: `readreceipt/api.py`
- Create: `readreceipt/main.py`
...
- [ ] **Step 3: Implement `readreceipt/api.py`**
...
- [ ] **Step 4: Implement minimal `readreceipt/main.py`**
...
```bash
uvicorn readreceipt.main:app --port 8000 &
sleep 2
...
```bash
git add readreceipt/api.py readreceipt/main.py tests/test_api.py
...
**Files:**
- Modify: `readreceipt/main.py`

- [ ] **Step 1: Replace `readreceipt/main.py` with the full app entry point**
...
export default function App() {
  return (
    <div className="h-full flex items-center justify-center text-text">
      <div className="text-2xl font-mono">ReadReceipt — coming up.</div>
    </div>
  );
}
```
...
- Page loads with ReadReceipt header, filter bar, two-pane layout.
...
**Files:**
- Modify: `readreceipt/main.py` (already mounts `frontend/dist/` if present — verify)
...
docker run --rm -p 8000:8000 \
  ...
  -e DATABASE_URL=sqlite:///./readreceipt.db \
  readreceipt &
sleep 6
curl -s http://localhost:8000/api/articles | head -c 40
curl -s http://localhost:8000/ | grep -q '<div id="root">'
docker stop $(docker ps -q --filter ancestor=readreceipt)
```
...
[deploy]
startCommand = "uvicorn readreceipt.main:app --host 0.0.0.0 --port $PORT"
...
```bash
gh repo create readreceipt --public --source=. --remote=origin --push
```
...
1. New Project → Deploy from GitHub repo → select `readreceipt`.
