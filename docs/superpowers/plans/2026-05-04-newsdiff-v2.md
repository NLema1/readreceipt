...
**Goal:** Build a Python/FastAPI + React service that scrapes Guardian/AP/Reuters article RSS feeds, snapshots articles over a 7-day window, classifies meaningful edits with Claude Haiku 4.5, and surfaces them in a dark-mode two-pane web UI. Deploy to Railway.
...
Find a current Guardian, AP, and Reuters article URL by hand (or by running `python -m newsdiff.cli --dry-run` and copying any URL). Then in a Python REPL:
...
urls = {
    ...
    "ap": "<paste a current AP URL>",
    "reuters": "<paste a current Reuters URL>",
}
...
const OUTLETS = ["guardian", "ap", "reuters"];
...
Track meaningful edits to news articles from Guardian, AP, and Reuters.
...
**Spec coverage:**
- Outlets (Guardian, AP, Reuters) — Task 1 (`feeds.yaml`), Task 12 (live verification).
